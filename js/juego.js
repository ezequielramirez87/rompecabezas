...
var instrucciones = ["Mueva las piezas hacia los lados.","Ordenalas como muestra en la imagen.","Usar las flechas del teclado"];
...
function ultimaDireccion(codigosDireccion){
  movimientos.push(codigosDireccion);
actualizarUltimoMovimiento(codigosDireccion);
};
/* Esta función va a chequear si el Rompecabezas esta en la posicion ganadora. 
Existen diferentes formas de hacer este chequeo a partir de la grilla. */
let movimientoGanador;
function chequearSiGano() {
  movimientoGanador=grilla.join();
  if (movimientoGanador==="1,2,3,4,5,6,7,8,9"){
...
function mostrarCartelGanador() {
  swal("GANASTE!!!","MUY BIEN LO LOGRASTE", "success");
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  let auxiliar = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2] = auxiliar;
}
...
function posicionValida(filaVacia, columnaVacia) {
  let posicionValida;

  if ((filaVacia>=0 && filaVacia<=2) && (columnaVacia>=0 && columnaVacia<=2)) {
    posicionValida = true;
  }
  else {
